# -*- coding: utf-8 -*-
"""PDF 페이지를 PNG로 렌더링.
사용법: python render_pdf.py YEAR DPI [pages]
  pages: "all" 또는 "1-10" 또는 "3,5,7" (1-기준)
출력: official-solutions/_page_renders/YEAR/pNNN.png
"""
import os, sys
import fitz
...
outdir = os.path.join(ROOT, "official-solutions", "_page_renders", year)
...
for p in pages:
    ...
    pix.save(out)
print(f"{year}: {len(pages)} pages rendered at {dpi}dpi -> {outdir}")
